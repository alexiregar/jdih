...
$dbname = "dev_v3_anggota";//diisi nama database
$varjson = array();
...
LEFT JOIN (SELECT data_pengarang.id, data_pengarang.id_dokumen, data_pengarang.nama_pengarang, pengarang.`name` AS teu FROM data_pengarang LEFT JOIN pengarang ON data_pengarang.nama_pengarang = pengarang.id) data_pengarang_fix ON (document.id = data_pengarang_fix.id)"; //query sql yang disesuaikan
...
$row_array->urlDetailPeraturan='http/s......./dokumen/view?id='.$row["sc_id"]; //berisi url halaman detail peraturan
